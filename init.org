...
(defun frame-align-left ()
  "Aligns the frame to the left side of the window"
  ;; Note: does not work well with monitors
  (interactive)
  (when (display-graphic-p)
    (set-frame-width (selected-frame) (/ (display-pixel-width) 2) nil t)
    (set-frame-height (selected-frame) (display-pixel-height) nil t)
    (set-frame-position (selected-frame) 0 0)))

(defun frame-align-right ()
  "Aligns the frame to the right side of the window"
  ;; Note: does not work well with monitors
  (interactive)
  (when (display-graphic-p)
      (set-frame-width (selected-frame) (/ (display-pixel-width) 2) nil t)
      (set-frame-height (selected-frame) (display-pixel-height) nil t)
      (set-frame-position (selected-frame) (- (display-pixel-width) (frame-pixel-width (selected-frame))) 0)))
...
(global-set-key (kbd "M-s-<left>") 'frame-align-left)
(global-set-key (kbd "M-s-<right>") 'frame-align-right)
...
(defun c-func-description (has-return-p) 
  ...
  (let* ((objects (split-string (thing-at-point 'line t) "[\(,\)]")) (type-name (split-string (car objects) "[ ]+"))
	 (field-n 1)
	 (func-decorations (car (split-string (car (last type-name)) "[a-zA-Z]" t)))
	 (func-undecorated (car (split-string (car (last type-name)) "[^a-zA-Z]" t)))
	 )
    (princ "func-undecorated: ")
    (princ (type-of func-undecorated))
    ...
    (yas-expand-snippet
     (concat "// " func-undecorated ": ${" (field-n) ":Describe Function}\n"
	     (if (check-against-list (cdr objects) (lambda (x) (string-match-p "[a-zA-Z]" x)))
		 (concat "//\n"
			 (concat-until-n (cdr objects) :n 1 :before "// " :after ": ${" :after-count ":Describe Argument}\n" :count-from 2)))
	     (if has-return-p (concat "//\n" "// return" 
				      (concat-until-n type-name :n 1 :before " " :add-before-last func-decorations) ": ${" (int-to-string (length objects)) ":Describe Return}\n\n")
	       "\n"))))
