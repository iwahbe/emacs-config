...
(use-package company
  ...
  (setq company-idle-delay 1) ; this makes company respond in real time (no delay)
...
This makes small improvements to how some emacs games work. So far it's only tetris.
#+BEGIN_SRC emacs-lisp
...
(defun tetris-restart ()
  ...
  (when (eql major-mode 'tetris-mode)
	...
	(progn
	  ...
	  (when (buffer-live-p "tetris-scores")
	    (kill-buffer-and-window "tetris-scores")
	    )
	  (tetris))
      (tetris-pause-game))))
(add-function :before (symbol-function 'tetris-end-game) (lambda () (if (y-or-n-p "Restart?:") (tetris))))
(add-hook 'tetris-mode-hook (lambda () (define-key tetris-mode-map (kbd "C-r") 'tetris-restart)))
#+END_SRC
