...
#+BEGIN_SRC emacs-lisp
(defmacro if-else (test &optional else) `(if ,test ,test ,else))

(cl-defun concat-until-n  (lst &key (n 0) (before "") (after "") (count-from 0 count-from-p) (after-count ""))
  "returns a concatonated list of list elements until only n are remaining with an optional count and other fields"
  (if (<= (length lst) n)
      ""   ; recursion base case 
    (concat ;otherwise
     (if-else before "")
...
(defun c-func-description ()
  "Inserts a c-function header description when called at or above a c function"
  (interactive)
  (while (equal (thing-at-point 'line t) "\n")
    (forward-line 1)
    )
  (let ((objects (split-string (thing-at-point 'line t) "[\(,\)]"))
	(field-n 1))
    ...
    (yas-expand-snippet
     (concat "// " (car objects) ": ${" (field-n) ":Describe Function}\n//\n"
     (concat-until-n (cdr objects) :n 1 :before "// " :after ": ${" :after-count ":Describe Argument}\n" :count-from 2)
     "//\n"
     "// return: ${" (int-to-string (length objects)) ":Describe Return}\n\n")
     )
    )
  )
(add-hook 'c-mode-hook (lambda () (local-set-key (kbd "C-c C-f") 'c-func-description)))
