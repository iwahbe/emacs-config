...
(use-package highlight-indent-guides
  :straight t
  :mode (("\\.py\\'" . highlight-indent-guides-mode))
  :config
...
(define-minor-mode autopep8
  ...
  (defvar autopep8-options "" "The options given to autopep8. Must not reroute output.")
  (defun autopep8-buffer ()
	...
	(let (
	  ...
	  (if (equal exec "")
	    ...
	    (progn
	      (call-process-region 1 (buffer-size)
				   exec nil tmp-buf nil (concat "-" autopep8-options))
	      (if (with-current-buffer tmp-buf (not (equal (buffer-substring 1 7) "[Errno")))
