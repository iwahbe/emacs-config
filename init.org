...
We set the custom package to it's own file, instead of polluting our init, and we can't put it in init.el, as it's blown away on tangle.
#+BEGIN_SRC emacs-lisp
...
We apply the kaolin-galaxy theme.
...
Used to setup modes derived from prog-mode.
We use my-prog-mode-called as a flag to indicate wither we load my-prog-mode again. This is necessary because otherwise it gets called repeatedly.
#+BEGIN_SRC emacs-lisp
...
(use-package irony
  ...
  (defun c-compile-buffer ()
    ...
    (let (
      ...
      (if compiler (compile (concat compiler " '" in "' -o '" out "'"))
...
(defun remove-blank-up (prefix) (interactive "P") (remove-blank-lines "//" 1  nil) (dotimes (a (- prefix 1)) (insert "\n")) (forward-line -1))
